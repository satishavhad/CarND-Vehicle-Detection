...
def draw_labeled_bboxes(img, labels):
    ...
    for car_number in range(1, labels[1]+1):
        ...
        x = size_x+bbox[0][0]
        y = size_y+bbox[0][1]
        asp = (y-350)/130.0+1.2 # Best rectangle aspect ratio for the box (coefficients from perspectieve measurements and experiments)
        if x>1050 or x<230:
            asp*=1.4
        ...
        if x > (-3.049*y+1809): #If the rectangle on the road, coordinates estimated from a test image
            ...
            if len(track_list) > 0:
                ...
                if min_d < 32:
                    ...
                    track_list_l[-1] = filt(track_list[ind], track_list_l[-1], 0.75)
    ...
    for bbox in boxes: # Iterate through the bounding boxes        
...
#    lane_line_img, ploty, left_line_detected, line_base_posi, right_line_detected, leftx, lefty, rightx, righty, left_fit, right_fit, right_fitx, left_fitx = slidinig_window(warped_img)
    ...
    y_start_stop = [400, 450]
    windows = slide_window(img, x_start_stop=[None, None], y_start_stop=y_start_stop, 
                        xy_window=(32, 32), xy_overlap=(0.85, 0.85))
    hot_windows = search_windows(img, windows, svc, X_scaler, color_space=color_space, 
                        spatial_size=spatial_size, hist_bins=hist_bins, 
                        orient=orient, pix_per_cell=pix_per_cell, 
                        cell_per_block=cell_per_block, 
